...
public class TestCToken {
        ...
        public void parse(CParseContext ctx) {
            ...
            while (tk.getType() != CToken.TK_EOF) {
                if (tk.getType() == CToken.TK_NUM) {
                    ctx.getIOContext()
                        .getOutStream().
                        println("Token=" + tk.toExplainString() +
                                "type=" + tk.getType() + " valule=" + tk.getIntValue());
                } else {
                    ctx.getIOContext()
                        .getOutStream()
                        .println("Token=" + tk.toExplainString() + "type=" + tk.getType());
                }
    ...
    public static void main(String[] args) {
        final String inFile = args[0]; // 適切なファイルを絶対パスで与えること
